...
public interface ProductoRepository extends JpaRepository<Producto, Long> {
    ...
    List<Producto> findByGrupoVarianteIdAndEstadoAndVisibleCatalogo(
        String grupoVarianteId, Integer estado, Boolean visibleCatalogo);
    ...
    /** Catálogo público: solo negocios aprobados y visibles (sin tienda propia de plataforma). */
    ...
    @Query("SELECT p FROM Producto p JOIN p.empresa e WHERE p.enOferta = true AND p.visibleCatalogo = true AND p.estado = 1 AND e.estadoEmpresa = 'ACTIVO' AND e.visibilidadPublica = true")
