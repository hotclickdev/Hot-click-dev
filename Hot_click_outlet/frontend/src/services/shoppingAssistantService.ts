...
export const shoppingAssistantService = {
  ...
  async getHistory(sesionId: string) {
  ...
  async expireSession(sesionId: string) {
    if (!sesionId) return
    try {
  ...
  async feedback(sesionId: string, msgIndex: number, rating: number) {
    if (!sesionId) return
    try {
