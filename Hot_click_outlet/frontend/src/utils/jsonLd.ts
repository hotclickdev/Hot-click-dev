...
/** Producto crudo o normalizado: solo los campos que usa el JSON-LD. */
type ProductoJsonLd = {
  id?: string | number
  nombre?: string
  sku?: string | null
...
type ProductLd = {
  ...
  sku: string
  mpn: string
  brand: { '@type': string; name: unknown }
 ...
 */
export function generateProductJsonLd(producto: ProductoJsonLd, urlBase: string) {
  ...
  if (producto.ratingPromedio && (producto.totalResenas ?? 0) > 0) {
    ld.aggregateRating = {
 ...
 */
export function generateWebsiteJsonLd(urlBase: string = SITE_URL) {
  return {
    '@context': 'https://schema.org',
    '@type': 'WebSite',
    name: SITE_NAME,
 ...
 */
export function generateOrganizationJsonLd(urlBase: string, socialUrls: string[] = []) {
  return {
    '@context': 'https://schema.org',
