 ...
 */
export function leerSeccionesColapsadas(userRole) {
  try {
    const raw = localStorage.getItem(CLAVE_SIDEBAR_COLAPSADO)
    if (raw == null) {
      return new Set(userRole === 'ADMIN' ? ADMIN_IT_SECCIONES_COLAPSADAS_POR_DEFECTO : [])
    }
...
function seccionAbastecimiento(t) {
  return [
 ...
 */
export function buildAdminItLinks(t) {
  return [
