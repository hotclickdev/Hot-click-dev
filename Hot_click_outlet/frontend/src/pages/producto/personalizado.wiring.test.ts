...
describe('productos personalizados — wiring estático', () => {
  ...
  it('panel de encargo compartido y presupuesto del cliente', () => {
    ...
    expect(helpers).toContain('Precio fijo')
    expect(helpers).toContain('Cotización')
    expect(helpers).toContain('Rango de precio')
    const campos = readFileSync(resolve(root, 'src/prototipo/compartido/CamposPersonalizadoProducto.tsx'), 'utf8')
    expect(campos).toContain('Forma de cobro')
  })
})
