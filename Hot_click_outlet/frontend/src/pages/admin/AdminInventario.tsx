...
type FilaInventario = {
  ...
  stock_actual?: number
  stock_minimo?: number
  demanda_diaria_avg?: number | string
...
type ColumnaInventario = {
  ...
  label: string
  color?: string
  render?: (v: unknown, row: FilaInventario) => ReactNode
}

type AbcResumenItem = { clase?: string; total?: number }

type DashboardInventario = {
  totalActivos?: number
  enRiesgoCount?: number
  lentosCount?: number
  enRiesgo?: FilaInventario[]
  lentos?: FilaInventario[]
  abcResumen?: AbcResumenItem[]
}

type TabInventario = 'riesgo' | 'lentos' | 'abc'

const fmt = (n: unknown) => new Intl.NumberFormat('es-CR').format(Number(n ?? 0))
const fmtDate = (ts: unknown) => ts ? String(ts).slice(0, 10) : '—'
              ...
              <tr key={i} style={{ backgroundColor: 'var(--hc-surface)', borderTop: '1px solid var(--hc-border)' }}>
                {cols.map(c => (
                  <td key={c.k} className="px-4 py-2.5" style={{ color: c.color ?? 'var(--hc-text)' }}>
                    {c.render ? c.render(row[c.k], row) : ((row[c.k] as ReactNode) ?? '—')}
                  </td>
                ))}
              </tr>
            ))}
...
export default function AdminInventario() {
  const [data, setData]       = useState<DashboardInventario | null>(null)
  const [cargando, setCargando] = useState(true)
  const [analizando, setAnalizando] = useState(false)
  const [error, setError]     = useState<string | null>(null)
  const [tab, setTab]         = useState<TabInventario>('riesgo') // riesgo | lentos | abc

  async function cargar() {
    setCargando(true); setError(null)
    try {
      const { data: d } = await inventarioService.getDashboard()
      setData(d as DashboardInventario)
      ...
      {cargando ? (
        ...
        <>
          ...
          <div className="flex gap-1 p-1 rounded-xl w-fit"
            ...
            {[
              { id: 'riesgo' as const, label: `En riesgo (${data.enRiesgoCount})` },
              { id: 'lentos' as const, label: `Mov. lento (${data.lentosCount})` },
              { id: 'abc' as const,    label: 'Clasificación ABC' },
            ].map(t => (
            ...
            <ProductTable
              ...
              cols={[
                ...
                { k: 'clasificacion_abc', label: 'ABC',
                  render: (v) => <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${ABC_STYLE[String(v ?? '?')] ?? ''}`}>{String(v ?? '?')}</span> },
