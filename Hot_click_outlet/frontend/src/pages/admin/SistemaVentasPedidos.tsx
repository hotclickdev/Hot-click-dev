...
import { listaPedidosDesdeRespuesta } from './ordenes/ordenesHelpers'
...
type TabVentasPedidos = 'pedidos' | 'ventas'

export default function SistemaVentasPedidos() {
  const [tab, setTab] = useState<TabVentasPedidos>('pedidos')
  ...
  const load = () => {
    ...
    orderService.getAll()
      .then(({ data }) => {
        setOrders(listaPedidosDesdeRespuesta(data))
      })
      .catch((err: unknown) => {
  ...
  const handleUpdate = (id: Id, fields: Partial<Pedido>) => setOrders(prev => prev.map(o => o.id === id ? { ...o, ...fields } : o))
  const handleDelete = (id: Id) => setOrders(prev => prev.filter(o => o.id !== id))
  const handleCreated = (newOrder: unknown) => {
    const pedido = newOrder as Pedido
    if (pedido?.id) setOrders(prev => [pedido, ...prev])
    else load()
  }
...
function AccionPrincipal({ tab, onCrearPedido }: { tab: TabVentasPedidos; onCrearPedido: () => void }) {
  if (tab === 'ventas') {
    return (
...
function TabsVentasPedidos({ tab, pendientes, onTab }: {
  tab: TabVentasPedidos
  pendientes: number
  onTab: (t: TabVentasPedidos) => void
}) {
  return (
