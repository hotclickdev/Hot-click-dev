...
import { mensajeErrorAsignar, type ClienteAsignar, type ItemAsignar } from './asignar/asignarHelpers'

export default function AdminAsignarProducto() {
  const navigate = useNavigate()
  const { showToast: toast } = useToast()
  const [paso, setPaso] = useState(0)
  const [cliente, setCliente] = useState<ClienteAsignar | null>(null)
  const [items, setItems] = useState<ItemAsignar[]>([])
  const [metodoPago, setMp] = useState<string>('EXTERNO')
  const [notas, setNotas] = useState('')
  const [enviando, setEnviando] = useState(false)

  const seleccionarCliente = (u: ClienteAsignar) => { setCliente(u); setPaso(1) }
  ...
  const confirmar = async () => {
    ...
    try {
      ...
      toast(mensajeErrorAsignar(err, 'Error al registrar la compra'), 'error')
