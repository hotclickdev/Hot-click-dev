...
function mensajeErrorMultipais(err: unknown, fallback: string): string {
...
function listaOVacio<T>(data: unknown): T[] {
  return Array.isArray(data) ? data as T[] : (data as T[] | undefined) ?? []
}

const MONEDAS = ['CRC','USD','MXN','COP','PEN','GTQ','HNL','NIO','EUR']

export default function AdminMultipais() {
  const [paises, setPaises]     = useState<PaisLatam[]>([])
  const [tasas, setTasas]       = useState<TasaCambio[]>([])
  const [guardando, setGuardando] = useState(false)
  const [ok, setOk]             = useState(false)
  const [error, setError]       = useState<string | null>(null)
  const [form, setForm]         = useState<ConfigMultipais | null>(null)
  ...
  useEffect(() => {
    Promise.all([
      ...
      setForm({ ...(c.data as ConfigMultipais) }); setPaises(listaOVacio<PaisLatam>(p.data)); setTasas(listaOVacio<TasaCambio>(t.data))
    }).catch(() => setError('No se pudo cargar la configuración'))
  }, [])

  const set = (k: keyof ConfigMultipais) => (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => setForm(p => ({ ...p, [k]: e.target?.value ?? e }))

  async function guardar(e: FormEvent) {
    e.preventDefault()
    setGuardando(true); setOk(false); setError(null)
    try {
      const { data } = await multipaisService.updateConfig(form as JsonBody)
      setForm({ ...(data as ConfigMultipais) }); setOk(true)
      setTimeout(() => setOk(false), 3000)
    } catch (err: unknown) {
      setError(mensajeErrorMultipais(err, 'Error al guardar'))
      ...
      {tasas.length > 0 && (
        <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--hc-border)' }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y"
            style={{ divideColor: 'var(--hc-border)' } as CSSProperties}>
            {tasas.map((t, i) => (
...
function VistaBanderaPais({ pais }: { pais?: PaisLatam }) {
  if (pais?.bandera) {
