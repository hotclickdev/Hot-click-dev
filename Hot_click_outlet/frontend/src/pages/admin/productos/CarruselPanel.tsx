...
export type CarruselPanelProps = {
...
function CarruselSlotOcupado({
  slot, index, total, onMover, onQuitar,
}: {
  slot: ProductoAdmin
  index: number
  total: number
...
function CarruselSlot({
  index, slot, totalOcupados, onMover, onQuitar,
}: {
  index: number
  slot: ProductoAdmin | undefined
  ...
  const estiloSlot: CSSProperties = {
    border: `1px solid ${slot ? slotBorder : 'var(--hc-border)'}`,
    background: slot ? slotColor : 'var(--hc-surface-2)',
    minHeight: 120,
  }
  return (
    <div
