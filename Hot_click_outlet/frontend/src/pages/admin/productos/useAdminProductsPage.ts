...
export function useAdminProductsPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<DeleteTargetProducto | null>(null)
  ...
  const [kardexProducto, setKardexProducto] = useState<ProductoAdmin | null>(null)
  ...
  const onSearch = (valor: string) => { setSearch(valor); setProdPage(0) }
