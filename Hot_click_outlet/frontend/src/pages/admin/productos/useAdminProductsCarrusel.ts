...
export function useAdminProductsCarrusel(deps: Pick<AdminProductsActionsDeps, 'products' | 'carruselSlots' | 'setProducts' | 'toast'> & {
  load: (page?: number) => void | Promise<void>
}) {
  const { products, carruselSlots, setProducts, toast, load } = deps

  const handleToggleCarrusel = useCallback(async (p: ProductoAdmin) => {
  ...
  const handleCarruselMover = useCallback(async (p: ProductoAdmin, dir: number) => {
  ...
  const handleToggleDestacado = useCallback(async (p: ProductoAdmin) => {
