...
export type AccionesProducto = {
  onEdit: (p: ProductoAdmin) => void
  onKardex: Dispatch<SetStateAction<ProductoAdmin | null>>
  onDelete: (id: number, nombre: string) => void
  onOferta: (p: ProductoAdmin) => void
  onOcultar: (p: ProductoAdmin) => void
  vistaSimple?: boolean
}

export type FilaProductoProps = AccionesProducto & {
  producto: ProductoAdmin
  isAdmin: boolean
  carruselSlots: ProductoAdmin[]
  t: TFunction
  onToggleDestacado: (p: ProductoAdmin) => void
  onToggleCarrusel: (p: ProductoAdmin) => void
}

function varianteCondicion(cond: string | undefined): BadgeProps['variant'] {
  return conditionVariant(cond ?? '') as BadgeProps['variant']
}
...
function varianteStock(stock: number): BadgeProps['variant'] {
  if (stock === 0) return 'danger'
  if (stock <= 3) return 'warning'
  return 'success'
}

function ProductoThumb({ producto, sizeClass }: { producto: ProductoAdmin; sizeClass: string }) {
...
function BotonDestacado({ producto, onToggle }: { producto: ProductoAdmin; onToggle: (p: ProductoAdmin) => void }) {
  return (
    <button type="button"
...
function estiloCarrusel(enCarrusel: boolean, lleno: boolean): CSSProperties {
...
function BotonCarrusel({ producto, carruselSlots, onToggle }: { producto: ProductoAdmin; carruselSlots: ProductoAdmin[]; onToggle: (p: ProductoAdmin) => void }) {
...
function tituloCarrusel(producto: ProductoAdmin, lleno: boolean) {
...
function CeldaSkuOMovimiento({ producto, vistaSimple }: { producto: ProductoAdmin; vistaSimple?: boolean }) {
  if (vistaSimple) {
...
function AccionesDesktop({ producto, t, onEdit, onKardex, onDelete, onOferta, onOcultar, vistaSimple }: AccionesProducto & { producto: ProductoAdmin; t: TFunction }) {
  if (vistaSimple) {
...
export default function FilaProducto({ producto, isAdmin, carruselSlots, t, onToggleDestacado, onToggleCarrusel, onEdit, onKardex, onDelete, onOferta, onOcultar, vistaSimple }: FilaProductoProps) {
  return (
    <motion.tr initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="transition-colors hover:bg-[var(--hc-surface-2)]" style={{ borderTop: '1px solid var(--hc-border)' }}>
      ...
      <td className="px-4 py-3">
        <Badge variant={varianteCondicion(producto.condicion)}>{conditionLabel(producto.condicion ?? '')}</Badge>
      </td>
      {isAdmin && (
...
export function TarjetaProducto({ producto, t, onEdit, onKardex, onDelete, onOferta, onOcultar, vistaSimple }: AccionesProducto & { producto: ProductoAdmin; t: TFunction }) {
  const visible = producto.visibleCatalogo !== false
  return (
