...
type FormGiftCard = { monto: string; fechaVencimiento: string; codigo: string }

function mensajeErrorGiftCard(err: unknown, fallback: string): string {
...
export default function AdminGiftCards() {
  const [cards, setCards]         = useState<GiftCardAdmin[]>([])
  const [cargando, setCargando]   = useState(true)
  const [error, setError]         = useState<string | null>(null)
  const [mostrarForm, setMostrarForm] = useState(false)
  const [creando, setCreando]     = useState(false)
  const [form, setForm]           = useState<FormGiftCard>({ monto: '', fechaVencimiento: '', codigo: '' })

  async function cargar() {
    setCargando(true)
    try {
      const { data } = await giftCardService.listar()
      setCards(Array.isArray(data) ? data as GiftCardAdmin[] : [])
  ...
  async function crear(e: FormEvent) {
    ...
    try {
      ...
      setError(mensajeErrorGiftCard(err, 'Error al crear la gift card'))
    } finally { setCreando(false) }
  }

  async function cancelar(id: Id) {
      ...
      {cargando ? (
        ...
        <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--hc-border)' }}>
          ...
          <table className="w-full min-w-[680px] text-sm">
            ...
            <tbody className="divide-y" style={{ divideColor: 'var(--hc-border)' } as CSSProperties}>
              {cards.map(gc => (
