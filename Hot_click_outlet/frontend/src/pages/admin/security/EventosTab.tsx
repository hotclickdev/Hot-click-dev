...
import { EVENT_LABEL, PERIODS, fmtDate, type PaginaSecurity, type SecurityEvent } from './securityHelpers'
import { Card, SeverityBadge } from './securityUi'
import TextoFlecha from '@/components/ui/TextoFlecha'

export default function EventosTab() {
  const [evPage, setEvPage]   = useState(0)
  const [evData, setEvData]   = useState<PaginaSecurity<SecurityEvent> | null>(null)
  ...
  const load = useCallback(async (p = 0) => {
    ...
    try {
      ...
      setEvData(data as PaginaSecurity<SecurityEvent>); setEvPage(p)
  ...
  const downloadCsv = () => {
    ...
    fetch(url, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .catch((err: unknown) => { console.error('[EventosTab] export CSV', err) })
  }
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <p className="text-xs" style={{ color: 'var(--hc-muted)' }}>
              {evData.totalElements} eventos · pág {(evData.page as number) + 1} de {evData.totalPages || 1}
            </p>
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => load(evPage + 1)} disabled={evPage >= ((evData.totalPages as number) - 1)}
                className="px-3 py-1.5 rounded-lg text-xs disabled:opacity-40"
                style={{ backgroundColor: 'var(--hc-card)', color: 'var(--hc-text)', border: '1px solid var(--hc-border)' }}>
