...
import { EVENT_LABEL, timeAgo, fmtDate, type EventosPorUsuario, type PaginaSecurity, type SecurityUsuario } from './securityHelpers'
import { Card, SeverityBadge } from './securityUi'
import TextoFlecha from '@/components/ui/TextoFlecha'

export default function UsuariosTab() {
  const [data, setData]         = useState<PaginaSecurity<SecurityUsuario> | null>(null)
  const [loading, setLoading]   = useState(true)
  const [page, setPage]         = useState(0)
  const [selected, setSelected] = useState<SecurityUsuario | null>(null)
  const [detalle, setDetalle]   = useState<EventosPorUsuario | null>(null)
  const [loadingDet, setLoadingDet] = useState(false)
  const [search, setSearch]     = useState('')

  const load = useCallback(async (p = 0) => {
    setLoading(true)
    try { const { data: d } = await securityService.getUsuarios({ page: p }); setData(d as PaginaSecurity<SecurityUsuario>); setPage(p) }
  ...
  const verDetalle = async (u: SecurityUsuario) => {
    setSelected(u); setDetalle(null); setLoadingDet(true)
    try { const { data: d } = await securityService.getEventosPorUsuario(u.correo as string); setDetalle(d as EventosPorUsuario) }
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => load(page + 1)} disabled={page >= ((data?.totalPages as number) - 1)}
                className="px-3 py-1.5 rounded-lg text-xs disabled:opacity-40"
                style={{ backgroundColor: 'var(--hc-card)', color: 'var(--hc-text)', border: '1px solid var(--hc-border)' }}>
