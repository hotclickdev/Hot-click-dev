...
import type {
  ...
  WizardForm,
  WizardMarca,
} from './nuevo-producto/wizardHelpers'
...
export default function AdminNuevoProducto() {
  ...
  const [imagenesFile, setImagenesFile] = useState<File[]>([])
  const [previewUrls, setPreviewUrls] = useState<string[]>([])

  const [analizando, setAnalizando] = useState(false)
  const [analizandoIdx, setAnalizandoIdx] = useState(-1)
  const [etiquetas, setEtiquetas] = useState<string[]>([])
  const [fuenteDetalles, setFuenteDetalles] = useState<string | null>(null)
  ...
  const [categories, setCategories] = useState<WizardCategoria[]>([])
  const [bodegas, setBodegas] = useState<WizardBodega[]>([])
  const [marcas, setMarcas] = useState<WizardMarca[]>([])
  ...
  const [seoLang, setSeoLang] = useState<SeoLangCode>('es')
  const [seoAuto, setSeoAuto] = useState<SeoAutoFlags>({ es: true, en: true, pt: true, fr: true })
  const [priceWarning, setPriceWarning] = useState(false)
  const [productoCreado, setProductoCreado] = useState<ProductoCreadoWizard | null>(null)
  ...
  useEffect(() => {
    ...
    setForm(p => { // eslint-disable-line react-hooks/set-state-in-effect -- SEO derivado del nombre/descripcion
      const next: SeoByLang = { ...p.seoByLang, ...auto }
  ...
  const handleAddFiles = useCallback((files: File[], silentlyDropped = 0) => {
  ...
  const setCampo: SetCampo = (f) => (e) => setForm((p) => ({ ...p, [f]: e.target.value }))
