...
import { ESTADO_COLOR_USUARIO, ESTADO_LABEL_USUARIO, ROL_CONFIG, type EmpresaMiembroTab } from './empresasHelpers'

function MiembroRow({ miembro }: { miembro: EmpresaMiembroTab }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ backgroundColor: 'var(--hc-surface-2)', border: '1px solid var(--hc-border)' }}>
      ...
      <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0 ${ESTADO_COLOR_USUARIO[miembro.estado ?? -1] ?? ''}`}>
...
export default function TabEquipo({ loading, equipo }: { loading: boolean; equipo: EmpresaMiembroTab[] | null }) {
