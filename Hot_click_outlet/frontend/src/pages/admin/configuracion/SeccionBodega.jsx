...
import { Block, FormGroup, StyledInput, F } from './configUi'
...
async function guardarBodega(e, ctx) {
  e.preventDefault()
  ...
  ctx.setSaving(true)
  try {
    if (ctx.editingId) await warehouseService.update(ctx.editingId, ctx.form)
    else await warehouseService.create(ctx.form)
    ctx.toast({ message: ctx.editingId ? 'Bodega actualizada' : 'Bodega creada', type: 'success' })
    ctx.setFormOpen(false)
    ctx.cargar()
  } catch (err) {
    ctx.toast({ message: err.response?.data?.message ?? 'No se pudo guardar', type: 'error' })
  } finally {
    ctx.setSaving(false)
  }
}

async function confirmarBorrado(borrar, setBorrar, setBodegas, toast) {
  if (!borrar) return
  const { id } = borrar
  setBorrar(null)
  try {
...
function TarjetaBodega({ bodega, onEdit, onDelete }) {
  return (
