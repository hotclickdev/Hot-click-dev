...
type CampoPerfil = 'nombre' | 'apellidoPaterno' | 'apellidoMaterno' | 'telefono'
...
export default function SeccionPerfil({ userId, userEmail, userName, setUserName, toast }: {
  ...
  useEffect(() => {
    ...
    adminService.getUsuario(userId)
      .then(({ data }) => {
        const u = (data as { data?: Record<string, string>; nombre?: string; apellidoPaterno?: string; apellidoMaterno?: string; telefono?: string }).data ?? (data as { nombre?: string; apellidoPaterno?: string; apellidoMaterno?: string; telefono?: string })
        setForm({
          nombre:          u.nombre          ?? '',
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    setSaving(true)
    try {
      await adminService.updateUsuario(userId as Id, form as unknown as JsonBody)
      setUserName(form.nombre)
      setSaved(true)
      toast({ message: t('adminConfig.perfilSaved'), type: 'success' })
      setTimeout(() => setSaved(false), 2500)
    } catch (err: unknown) {
      toast({ message: mensajeErrorConfig(err, t('adminConfig.perfilErrorSave')), type: 'error' })
