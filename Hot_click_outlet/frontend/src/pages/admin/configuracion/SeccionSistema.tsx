...
import {
  ...
  DBIcon, ServerIcon, MailIcon, CardIcon, mensajeErrorConfig,
} from './configUi'

type ToastFn = (opts: { message: string; type?: 'success' | 'error' | 'warning' | 'info' }) => void

export default function SeccionSistema({ toast }: { toast: ToastFn }) {
  ...
  const handleReset = async () => {
    ...
    catch (err: unknown) { toast({ message: mensajeErrorConfig(err, t('adminConfig.sysResetError')), type: 'error' }) }
      ...
      <Block label={t('adminConfig.sysHealthTitle')} sublabel={t('adminConfig.sysHealthSubtitle')}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <button type="button" onClick={checkHealth} disabled={checking} className="cfg-btn cfg-btn-ghost" style={{ fontSize: '12px', padding: '7px 14px' }}>
            {checking ? <Spinner size={'xs' as 'sm'} /> : <RefreshIcon style={{ width: '14px', height: '14px' }} />}
            ...
            <div key={titleKey}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '12px 0' }}>
                ...
                <button type="button" onClick={action} disabled={loading} className="cfg-btn cfg-btn-ghost" style={{ flexShrink: 0, fontSize: '12px', padding: '7px 12px' }}>
                  {loading ? <Spinner size={'xs' as 'sm'} /> : null}
                  {t(labelKey)}
                </button>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div key={label}
              ...
              onMouseEnter={(e: MouseEvent<HTMLDivElement>) => { e.currentTarget.style.borderColor = `color-mix(in srgb, ${color} 25%, transparent)` }}
              onMouseLeave={(e: MouseEvent<HTMLDivElement>) => { e.currentTarget.style.borderColor = 'var(--hc-border)' }}>
          ...
          <div style={{ width: '100%', maxWidth: '440px', borderRadius: '18px', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.25)', background: 'var(--hc-surface)', border: '1px solid rgba(239,68,68,0.3)' }}>
            ...
            <div style={{ padding: '0 24px 24px', display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              ...
              <button type="button" onClick={handleReset} disabled={resetInput !== 'ELIMINAR' || resetting} className="cfg-btn"
                ...
                {resetting ? <Spinner size={'xs' as 'sm'} /> : <SkullIcon style={{ width: '14px', height: '14px' }} />}
...
function colorHealthDot(healthOk: boolean | null) {
  if (healthOk === null) return 'var(--hc-muted)'
  return healthOk ? '#22c55e' : '#ef4444'
}

function colorHealthText(healthOk: boolean | null) {
  if (healthOk === null) return 'var(--hc-muted)'
  return healthOk ? '#4ade80' : '#f87171'
}

function textoHealth(healthOk: boolean | null, t: TFunction) {
