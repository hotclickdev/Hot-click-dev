...
type TwoFaStatus = {
  data?: { enabled?: boolean; totpEnabled?: boolean; emailOtpEnabled?: boolean }
  enabled?: boolean
  totpEnabled?: boolean
...
export default function SeccionSeguridad({ refreshToken, toast, onTwoFAChange }: {
  ...
  const fetchStatus = () => {
    authService.get2FAStatus()
      .then(({ data }) => {
        const s = (data as TwoFaStatus).data ?? (data as TwoFaStatus)
      ...
      .catch((err: unknown) => { console.error('[SeccionSeguridad] 2FA status', err) })
...
function colorScoreSeguridad(score: number) {
