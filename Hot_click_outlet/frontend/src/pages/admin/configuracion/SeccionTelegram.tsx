...
import {
  F, Block, SectionHeader, ShoppingIcon, AlertIcon, RefreshIcon, DBIcon, CheckIcon, SendIcon, mensajeErrorTelegram,
} from './configUi'
...
type TelegramLink = { deepLink: string; expiraEnMin?: number }
type TelegramMiembro = { usuarioId: Id; nombre?: string; correo?: string; telegramUsername?: string }

export default function SeccionTelegram({ toast }: { toast: ToastFn }) {
  const [estado, setEstado] = useState<TelegramEstado | null>(null)
  const [link, setLink] = useState<TelegramLink | null>(null)
  const [qr, setQr] = useState<string | null>(null)
  const [equipo, setEquipo] = useState<TelegramMiembro[]>([])
  ...
  const conectar = async () => {
    ...
    try {
      ...
      setLink(data as TelegramLink)
      try { setQr(await QRCode.toDataURL((data as TelegramLink).deepLink, { margin: 1, width: 180 })) } catch { setQr(null) }
    } catch (e: unknown) {
      toast({ message: mensajeErrorTelegram(e, 'No se pudo generar el código de vinculación'), type: 'error' })
    }
    setGenerando(false)
  }
  ...
  const revocar = async (usuarioId: Id) => {
    try {
