...
export default function TfaSetupStep({
  ...
  t: TFunction
  qrData: { secret?: string; qrUri?: string }
  ...
  return (
    <div style={{ borderRadius: '12px', padding: '16px', background: 'rgba(23,71,168,0.06)', border: `1px solid ${copiedAll ? 'rgba(34,197,94,0.3)' : 'rgba(23,71,168,0.18)'}`, display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', gap: '10px' }}>
        <button type="button" onClick={onActivate} disabled={codeStr.length !== 6 || working} className="cfg-btn cfg-btn-primary">{working ? <Spinner size={'xs' as 'sm'} /> : <CheckIcon style={{ width: '14px', height: '14px' }} />}{t('adminConfig.tfaActivateSubmit')}</button>
