...
export default function ConteoEfectivo({ label, onTotal, totalColor = '#34d399' }: {
  ...
  const [qtys, setQtys] = useState<Record<number, string | number>>(() => Object.fromEntries(DENOM.map(d => [d.v, ''])))

  const setQty = (v: number, val: string | number) => {
    const next = { ...qtys, [v]: val }
    setQtys(next)
    onTotal(DENOM.reduce((s, d) => s + (Number.parseInt(String(next[d.v])) || 0) * d.v, 0))
  }

  const total = DENOM.reduce((s, d) => s + (Number.parseInt(String(qtys[d.v])) || 0) * d.v, 0)
          ...
          return (
            <div key={d.v} className="flex items-center gap-2 sm:gap-3 px-3 py-2 border-b last:border-0"
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button type="button" onClick={() => setQty(d.v, Math.max(0, (Number.parseInt(String(qty)) || 0) - 1))}
                ...
                />
                <button type="button" onClick={() => setQty(d.v, (Number.parseInt(String(qty)) || 0) + 1)}
                  className="w-7 h-7 rounded-lg font-bold text-base flex items-center justify-center transition-colors hover:bg-white/10"
                  style={{ color: 'var(--hc-muted)' }}>+</button>
              </div>
