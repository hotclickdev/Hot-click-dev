...
export default function ServicioDetalleDrawer({
  ...
  saving: boolean
  onClose: () => void
  onNuevoEstado: (e: string) => void
  onNotas: (v: string) => void
  onGuardar: () => void
  onEliminar: (id: Id) => void
  onFotoModal: (url: string) => void
}) {
      ...
      <motion.div
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {waLinkServicio(selected) && (
            <a href={waLinkServicio(selected) ?? undefined} target="_blank" rel="noopener noreferrer"
...
export function ServicioFotoLightbox({ fotoModal, onClose }: { fotoModal: string | null; onClose: () => void }) {
