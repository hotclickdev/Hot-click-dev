import { useState, useCallback, useRef, type ChangeEvent, type Dispatch, type SetStateAction } from 'react'
...
export default function BuscarCliente({ onSelect }: { onSelect: (usuario: ClienteAsignar) => void }) {
  const [q, setQ] = useState('')
  const [resultados, setResultados] = useState<ClienteAsignar[]>([])
  const [buscando, setBuscando] = useState(false)
  const [modo, setModo] = useState<TabCliente>('buscar')
  const [form, setForm] = useState<FormCliente>(FORM_CLIENTE_VACIO)
  ...
  const handleQ = (e: ChangeEvent<HTMLInputElement>) => { setQ(e.target.value); buscar(e.target.value) }
  ...
  const crear = async () => {
    ...
    setCreando(true)
    try {
      const { data } = await asignarService.crearCliente({ ...form } as JsonBody)
      toast('Cliente creado correctamente', 'success')
      onSelect(clienteDesdeCrear(data))
    } catch (err: unknown) {
      toast(mensajeErrorAsignar(err, 'Error al crear cliente'), 'error')
...
function PanelBuscar({ q, onQ, buscando, resultados, onSelect, onCrear }: {
  q: string
  onQ: (e: ChangeEvent<HTMLInputElement>) => void
  buscando: boolean
  resultados: ClienteAsignar[]
  onSelect: (usuario: ClienteAsignar) => void
  onCrear: () => void
}) {
  return (
...
function FormCrearCliente({ form, onChange, creando, onCrear }: {
  form: FormCliente
  onChange: Dispatch<SetStateAction<FormCliente>>
  creando: boolean
  onCrear: () => void
}) {
  return (
