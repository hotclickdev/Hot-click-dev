...
import type { CostoHistorial, ProveedorAdmin, ProveedorForm } from './proveedores/proveedoresHelpers'
...
export default function AdminProveedores() {
  ...
  const [editing, setEditing]         = useState<ProveedorAdmin | null>(null)
  const [form, setForm]               = useState<ProveedorForm>(EMPTY_PROVEEDOR)
  const [saving, setSaving]           = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<ProveedorAdmin | null>(null)
  const [costosTarget, setCostosTarget] = useState<ProveedorAdmin | null>(null)
  const [historial, setHistorial]       = useState<CostoHistorial[]>([])
  const [loadingHistorial, setLoadingHistorial] = useState(false)

  const load = () => {
    setLoading(true)
    compraService.listarProveedores()
      .then((data: unknown) => setProveedores(Array.isArray(data) ? data as ProveedorAdmin[] : []))
      .catch((err: unknown) => { void err; showToast('Error al cargar proveedores', 'error') })
  ...
  const setField = (field: keyof ProveedorForm, val: string) => setForm(f => ({ ...f, [field]: val }))
