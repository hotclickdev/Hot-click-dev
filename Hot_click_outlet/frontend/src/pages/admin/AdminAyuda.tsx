...
function mensajeErrorAyuda(err: unknown, fallback: string): string {
  ...
  const enviar = async (e: FormEvent) => {
    ...
    try {
      ...
      if (file) {
        ...
        const payload = data as { data?: { url?: string }; url?: string } | undefined
        fotoUrl = payload?.data?.url ?? payload?.url ?? null
        setSubiendoFoto(false)
      }
      await soporteService.crearTicket({ titulo: titulo.trim(), descripcion: descripcion.trim(), fotoUrl: fotoUrl as string | undefined })
      setEnviado(true)
    } catch (err: unknown) {
      showToast(mensajeErrorAyuda(err, 'Error al enviar el reporte'), 'error')
    } finally {
      setEnviando(false)
      setSubiendoFoto(false)
    }
