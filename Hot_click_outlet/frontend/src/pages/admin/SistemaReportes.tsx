...
const DIA_VACIO: DiaConTotal = { total: 0, label: '—', iso: '', dow: 0, labelLargo: '—' }
...
export default function SistemaReportes() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      productService.adminGetAll(0, 500).catch((err: unknown) => {
        console.error('[SistemaReportes] productos', err)
        fallos += 1
        return { data: [] }
      }),
    ]).then(([{ data: vs }, { data: ps }]) => {
      setVentas(listaOPagina<VentaReporte>(vs))
      setProductos(listaProductosDesdeRespuesta(ps))
      if (fallos > 0) {
  ...
  const mejorDia = porDia.reduce((max, d) => d.total > max.total ? d : max, porDia[0] ?? DIA_VACIO)
