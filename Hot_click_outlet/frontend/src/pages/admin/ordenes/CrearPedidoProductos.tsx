...
import type { ItemFormPedido, ProductoCrearPedido } from './ordenesHelpers'
...
export default function CrearPedidoProductos({ prodRef, prodSearch, showProdDrop, filteredProds, items, inp, onSearch, onAdd, onRemove, onUpdateItem }: {
  ...
  prodSearch: string
  showProdDrop: boolean
  filteredProds: ProductoCrearPedido[]
  items: ItemFormPedido[]
  inp: CSSProperties
  onSearch: (v: string) => void
  onAdd: (p: ProductoCrearPedido) => void
  onRemove: (id: Id) => void
  onUpdateItem: (id: Id, field: 'cantidad' | 'precioUnitario', val: string) => void
}) {
            ...
            <div key={item.productoId} className="rounded-xl px-3 py-2.5 space-y-2"
              ...
              <div className="flex gap-2">
                ...
                <span className="text-xs text-[var(--hc-muted)] self-center shrink-0">
                  = {formatPrice((Number.parseInt(item.precioUnitario as string) || 0) * (Number.parseInt(item.cantidad as string) || 0))}
                </span>
              </div>
            </div>
          ))}
