...
export default function OrderCardExpanded({
  ...
  showOver: boolean
  override: string
  needsEnvioForm: boolean
  onPending: Dispatch<SetStateAction<string | null>>
  ...
  onSendEmail: () => void
  onShowOver: Dispatch<SetStateAction<boolean>>
            ...
            {items.map((item, i) => (
              <div key={i} className="flex items-center gap-3 rounded-xl px-3 py-2.5"
                ...
                <span className="text-sm font-medium text-[var(--hc-text)] shrink-0">
                  {formatPrice(((item.precioUnitario ?? 0) as number) * (item.cantidad as number))}
                </span>
              </div>
            ))}
          </div>
