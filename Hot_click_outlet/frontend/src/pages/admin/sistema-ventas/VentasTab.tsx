...
import {
  ...
  type VentaPosHistorial,
} from './ventasPedidosHelpers'
import TextoFlecha from '@/components/ui/TextoFlecha'

export default function VentasTab() {
  const { showToast } = useToast()
  const [ventas, setVentas] = useState<VentaPosHistorial[]>([])
  const [loading, setLoading] = useState(true)
  const [periodo, setPeriodo] = useState('hoy')

  useEffect(() => {
    posService.historial()
      .then(res => setVentas(listaVentasPos(res)))
      .catch((err: unknown) => {
  ...
  const filtradas = useMemo(
    () => ventas.filter(v => dentroDelPeriodo(v.fechaPedido, periodo))
      .sort((a, b) => new Date(b.fechaPedido ?? 0).getTime() - new Date(a.fechaPedido ?? 0).getTime()),
    [ventas, periodo]
  )
          ...
          <tbody>
            {ventas.map(v => (
              <tr key={v.id} className="transition-colors hover:bg-[var(--hc-surface-2)]" style={{ borderTop: '1px solid var(--hc-border)' }}>
                ...
                <td className="px-4 py-3" style={{ color: 'var(--hc-muted)' }}>{formatDate(v.fechaPedido ?? 0)}</td>
