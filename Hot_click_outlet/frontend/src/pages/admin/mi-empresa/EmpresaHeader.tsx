import { PLAN_COLOR, ESTADO_COLOR, fechaPerfil, perfilCompletitud, type EmpresaPerfil, type FormularioEmpresa } from './miEmpresaHelpers'
import CopiarLinkTienda from '@/components/sistema/CopiarLinkTienda'
import { tiendaEsPublica } from '@/utils/rutaTienda'
import TrustGlyph from '@/components/ui/TrustGlyph'

function CompletitudCard({ form }: { form: FormularioEmpresa }) {
    ...
    <>
      ...
      <div className="rounded-xl p-4 flex flex-wrap gap-3 items-center"
        ...
        <div className="flex gap-2 flex-wrap">
          <span className={`text-xs font-semibold px-2 py-1 rounded-full ${PLAN_COLOR[empresa.planSaas as string] ?? ''}`}>
            {empresa.planSaas}
          </span>
          <span className={`text-xs font-semibold px-2 py-1 rounded-full ${ESTADO_COLOR[empresa.estadoEmpresa as string] ?? ''}`}>
