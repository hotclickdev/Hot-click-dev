...
type TabForecast = 'ingresos' | 'unidades'

const fmt = (n: number | string | null | undefined) => new Intl.NumberFormat('es-CR').format(Math.round(Number(n ?? 0)))
...
export default function AdminForecast() {
  const [data, setData]           = useState<DashboardForecast | null>(null)
  const [cargando, setCargando]   = useState(true)
  const [generando, setGenerando] = useState(false)
  const [error, setError]         = useState<string | null>(null)
  const [tab, setTab]             = useState<TabForecast>('ingresos') // ingresos | unidades

  async function cargar() {
    setCargando(true); setError(null)
    try {
      const { data: d } = await forecastService.getDashboard()
      setData(d as DashboardForecast)
          ...
          <div className="rounded-2xl p-5 space-y-4"
            ...
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <div className="flex gap-1 p-1 rounded-xl" style={{ backgroundColor: 'var(--hc-bg)' }}>
                {(['ingresos', 'unidades'] as const).map(t => (
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-1.5 rounded-full" style={{ backgroundColor: 'rgba(255,255,255,0.1)' }}>
                            <div className="h-full rounded-full"
                              style={{
                                width: `${Number(f.confianza)}%`,
                                backgroundColor: Number(f.confianza) >= 70 ? '#34d399' : Number(f.confianza) >= 40 ? '#fbbf24' : '#f87171',
                              }} />
                          </div>
