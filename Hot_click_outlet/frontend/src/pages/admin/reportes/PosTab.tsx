...
import { SUCCESS, INFO, cardStyle, type VentaReporte } from './reportesHelpers'

type PosTabProps = {
  loading: boolean
  posFiltradas: VentaReporte[]
  posTx: number
  posTotal: number
  posTicket: number
}

export default function PosTab({ loading, posFiltradas, posTx, posTotal, posTicket }: PosTabProps) {
              ...
              <tbody>
                {posFiltradas.slice(0, 100).map(v => (
                  <tr key={v.id} className="transition-colors hover:bg-[var(--hc-surface-2)]" style={{ borderTop: '1px solid var(--hc-border)' }}>
                    ...
                    <td className="px-4 py-3 text-xs" style={{ color: 'var(--hc-muted)' }}>{formatDate(v.fechaPedido as string)}</td>
