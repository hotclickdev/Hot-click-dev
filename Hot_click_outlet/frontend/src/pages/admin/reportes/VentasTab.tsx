...
type VentasTabProps = {
  ...
  metodoPago: string
  onMetodoPago: (value: string) => void
  estado: string
  onEstado: (value: string) => void
  ventas: VentaReporte[]
  filtered: VentaReporte[]
  totalIngresos: number
  totalEnvios: number
  totalProductos: number
  ticketPromedio: number
  completadas: VentaReporte[]
  loading: boolean
  tablePage: number
  onTablePage: Dispatch<SetStateAction<number>>
}
...
export default function VentasTab({
  ...
  const metodos = [...new Set(ventas.map(v => v.metodoPago).filter((m): m is string => Boolean(m)))]
  const estados = [...new Set(ventas.map(v => v.estado).filter((s): s is string => Boolean(s)))]
                  ...
                  : paginated.map(v => {
                    ...
                    return (
                      <tr key={v.id} className="transition-colors hover:bg-[var(--hc-surface-2)]" style={{ borderTop: '1px solid var(--hc-border)' }}>
                        ...
                        <td className="px-4 py-3 text-xs" style={{ color: 'var(--hc-muted)' }}>{formatDate((v.fechaCreacion ?? v.fechaPedido) as string)}</td>
