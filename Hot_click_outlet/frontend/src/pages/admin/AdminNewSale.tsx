...
export default function AdminNewSale() {
  ...
  const [tab, setTab] = useState<TabVentaId>('cliente')
  const [products, setProducts] = useState<Producto[]>([])
  const [clients, setClients] = useState<ClienteVenta[]>([])
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [createdOrder, setCreatedOrder] = useState<CreatedOrderVenta | null>(null)

  const [items, setItems] = useState<ItemCarritoVenta[]>([])
  ...
  const [waTab, setWaTab] = useState<WaTabKey>('formal')
  const [waTexts, setWaTexts] = useState<CotizacionTemplates | null>(null)
  ...
  useEffect(() => {
    Promise.all([
      ...
      setProducts((prods as { content?: Producto[] }).content ?? (prods as Producto[]) ?? [])
      setClients(Array.isArray(cl) ? cl as ClienteVenta[] : [])
