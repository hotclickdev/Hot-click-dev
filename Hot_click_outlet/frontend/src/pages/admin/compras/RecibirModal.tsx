...
import { mensajeErrorCompra } from './comprasHelpers'
import type { OrdenCompra } from './comprasHelpers'
...
export default function RecibirModal({ orden, onClose, onDone }: {
  ...
  const [cantidades, setCantidades] = useState<Record<string, number>>(
    (orden.items ?? []).reduce((acc, it) => ({ ...acc, [String(it.id)]: it.cantidad - it.cantidadRecibida }), {} as Record<string, number>),
  )
  const [saving, setSaving] = useState(false)

  const handleConfirmar = async () => {
    const items = Object.entries(cantidades)
      .map(([itemId, cantidadRecibida]) => ({ itemId: Number(itemId), cantidadRecibida: Number.parseInt(String(cantidadRecibida)) || 0 }))
    ...
    setSaving(true)
    try {
      await compraService.recibir(orden.id, { items } as JsonBody)
      showToast('Recepción registrada y stock actualizado', 'success')
      onDone()
    } catch (err: unknown) {
      showToast(mensajeErrorCompra(err, 'Error al recibir'), 'error')
    } finally {
      setSaving(false)
    }
  }
