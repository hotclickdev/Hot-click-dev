...
export default function AdminCategories() {
  ...
  const [form, setForm] = useState<FormularioCategoria>(FORMULARIO_CATEGORIA_VACIO)
  const [saving, setSaving] = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<DeleteTargetCategoria | null>(null)
  ...
  function abrirNueva(padreId: Id | '' = '') {
    setEditing(null)
    setForm(formularioNuevaCategoria(padreId))
    setModalOpen(true)
  }

  function abrirEdicion(categoria: CategoriaAdmin) {
    setEditing(categoria)
    setForm(formularioDesdeCategoria(categoria))
    setModalOpen(true)
  }

  function actualizarCampo(campo: keyof FormularioCategoria) {
    return (evento: { target: { value: string } }) => setForm((prev) => ({ ...prev, [campo]: evento.target.value }))
  }

  async function guardarCategoria(evento: FormEvent) {
  ...
  async function importarCategorias(filas: FilaImport[]) {
...
function CategoriasContenido({ loading, arbol, categorias, onNueva, onEdit, onDelete, onAddSub }: {
  ...
  onDelete: (nodo: CategoriaNodo) => void
  onAddSub: (padre: CategoriaNodo) => void
}) {
  if (loading) {
