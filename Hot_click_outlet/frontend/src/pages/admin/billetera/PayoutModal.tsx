...
export default function PayoutModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const set = (f: keyof PayoutForm) => (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) =>
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await walletService.solicitarPayout({ ...form, monto: Number(form.monto) } as JsonBody)
      showToast('Solicitud de retiro enviada', 'success')
      onSaved()
    } catch (e: unknown) {
      showToast(mensajeErrorBilletera(e, 'Error al solicitar retiro'), 'error')
    } finally {
      setSaving(false)
    }
  }
