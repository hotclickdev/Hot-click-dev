...
import { mensajeErrorCompra } from './compras/comprasHelpers'
import type { ProveedorAdmin } from './proveedores/proveedoresHelpers'
import type { Producto } from '@/types/producto'
import type { JsonBody } from '@/types/api'

const fmt = (n: number | null | undefined) => new Intl.NumberFormat('es-CR').format(n ?? 0)

type LineaCompra = { productoId: string; cantidad: number | string; precioUnitario: number | string }
...
export default function AdminNuevaCompra() {
  ...
  const [items,       setItems]       = useState<LineaCompra[]>([{ productoId: '', cantidad: 1, precioUnitario: 0 }])
  ...
  useEffect(() => {
    Promise.all([
      ...
      setProveedores(Array.isArray(prov) ? prov as ProveedorAdmin[] : [])
      const data = prod?.data as { content?: Producto[] } | Producto[] | undefined
      const lista = (data && typeof data === 'object' && 'content' in data && data.content)
        ? data.content
        : (Array.isArray(data) ? data : [])
      setProductos(lista)
    }).catch((err: unknown) => { void err; showToast('Error al cargar datos', 'error') })
  ...
  const updateItem = <K extends keyof LineaCompra>(i: number, field: K, val: LineaCompra[K]) =>
  ...
  const total = items.reduce((s, it) => {
    const n = Number.parseInt(String(it.cantidad)) || 0
    const p = Number.parseInt(String(it.precioUnitario)) || 0
    return s + n * p
  }, 0)

  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault()
    const validItems = items.filter(it => it.productoId && String(it.productoId).trim() !== '' && Number.parseInt(String(it.cantidad)) > 0)
    ...
    try {
      await compraService.crear({
        ...
        items: validItems.map(it => ({
          ...
          cantidad:      Number.parseInt(String(it.cantidad)),
          precioUnitario: Number.parseInt(String(it.precioUnitario)) || 0,
        })),
      } as JsonBody)
      showToast('Orden de compra creada', 'success')
      navigate('/admin/compras')
    } catch (err: unknown) {
      showToast(mensajeErrorCompra(err, 'Error al crear la orden'), 'error')
    } finally {
      setSaving(false)
    }
  }
  ...
  const inpStyle: CSSProperties = { backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--hc-text)' }
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="rounded-2xl p-5 space-y-3"
        ...
        {items.map((item, i) => {
          const sub = (Number.parseInt(String(item.cantidad)) || 0) * (Number.parseInt(String(item.precioUnitario)) || 0)
          return (
