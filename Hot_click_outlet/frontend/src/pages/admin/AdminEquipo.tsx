...
export default function AdminEquipo() {
  ...
  async function obtenerMiembros() {
    ...
    return (data as { data?: MiembroEquipo[] }).data ?? []
  }
  ...
  function patchForm(patch: Partial<FormularioEquipo>) {
    setForm((s) => ({ ...s, ...patch }))
  }
  ...
  async function cambiarRol(id: Id, rolEnEmpresa: string) {
    setSaving(true)
    try {
      ...
      toast({ message: mensajeErrorEquipo(err, 'Error al cambiar rol'), type: 'error' })
    } finally {
      setSaving(false)
    }
  }
  ...
  async function eliminar(id: Id) {
    ...
    try {
      ...
      toast({ message: mensajeErrorEquipo(err, 'Error al eliminar'), type: 'error' })
    } finally {
      setSaving(false)
    }
  }
