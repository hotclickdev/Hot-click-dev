...
import { EMPTY_BLOG_FORM, type BlogForm } from './blogHelpers'
...
function FieldLabel({ children, count }: { children: ReactNode; count?: string | number | null }) {
  return (
...
export type BlogModalProps = {
  entrada: BlogForm | null
  onSave: (form: BlogForm) => Promise<void>
  onClose: () => void
}

export default function BlogModal({ entrada, onSave, onClose }: BlogModalProps) {
  const [form, setForm] = useState<BlogForm>(entrada ?? EMPTY_BLOG_FORM)
  const [saving, setSaving] = useState(false)
  const [focused, setFocused] = useState<string | null>(null)
  const [imgError, setImgError] = useState(false)
  const { showToast } = useToast()

  const set = (k: 'titulo' | 'resumen' | 'contenido' | 'imagenUrl') =>
    (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => setForm(f => ({ ...f, [k]: e.target.value }))
  ...
  const focusHandlers = (key: string) => ({ onFocus: () => setFocused(key), onBlur: () => setFocused(null) })
