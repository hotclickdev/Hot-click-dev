...
export default function AdminHomepage() {
  ...
  useEffect(() => {
    categoriaService.getAll()
      .then(r => {
        const data: unknown = (r.data as { data?: unknown } | undefined)?.data ?? r.data
        setCategories(Array.isArray(data) ? data as CategoriaHomepage[] : [])
      })
    ...
    <div className="max-w-3xl mx-auto py-8 px-4 space-y-8">
      ...
      <section className="rounded-2xl p-6 space-y-4" style={{ background: 'var(--hc-surface)', border: '1px solid var(--hc-border)' }}>
        ...
        <p className="text-xs px-1" style={{ color: 'var(--hc-muted)' }}>
          Activo: <TextoCamino partes={config.heroSections.map(id => HERO_OPTIONS.find(o => o.id === id)?.label).filter((x): x is string => Boolean(x))} />
        </p>
      </section>
