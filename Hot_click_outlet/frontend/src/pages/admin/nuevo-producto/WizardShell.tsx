...
import type { WizardView } from './wizardHelpers'
...
function ValidationBanner({ validationMsg }: { validationMsg: string }) {
...
function WizardFooter({ wizard }: { wizard: WizardView }) {
  const {
      ...
      <div className="flex-1 min-w-0 flex flex-col px-4 py-6 md:px-8 md:py-8">
        <div className="w-full max-w-lg mx-auto lg:mx-0 flex-1 flex flex-col">
          ...
          <div className="mt-6 mb-6">
            <h1 className="text-2xl font-bold" style={{ fontFamily: 'var(--font-display)', color: 'var(--hc-text)' }}>{paso.title}</h1>
            {paso.subtitle && (
              <p className="text-sm mt-1" style={{ color: 'var(--hc-muted)' }}>{paso.subtitle}</p>
            )}
          </div>
...
function etiquetaPublicar(saving: boolean, sinBodegas: boolean) {
  if (saving) {
