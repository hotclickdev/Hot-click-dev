...
import type { PluginAdmin, PluginLog } from './pluginsHelpers'
import CloseIcon from '@/components/ui/CloseIcon'

export default function LogModal({ plugin, onClose }: { plugin: PluginAdmin; onClose: () => void }) {
  const [logs, setLogs] = useState<PluginLog[]>([])
  const [cargando, setCargando] = useState(true)

  useEffect(() => {
    pluginService.getEventos(plugin.id)
      .then(({ data }) => setLogs(Array.isArray(data) ? data as PluginLog[] : []))
      .catch((err: unknown) => { console.error('[LogModal] eventos', err) })
...
function contenidoLogs(cargando: boolean, logs: PluginLog[]) {
  if (cargando) {
    return (
