import { useState, useEffect, type ChangeEvent, type CSSProperties, type FormEvent } from 'react'
...
type ConvenioAdmin = ConvenioForm & { id: Id }

const empty: ConvenioForm = { nombre: '', descripcion: '', logoUrl: '', urlWeb: '', activo: true }

function listaConveniosAdmin(data: unknown): ConvenioAdmin[] {
  if (!data || typeof data !== 'object') return []
  const inner = (data as { data?: unknown }).data
  return Array.isArray(inner) ? inner as ConvenioAdmin[] : []
}

function Modal({ convenio, onSave, onClose }: {
  convenio: ConvenioAdmin | null
  onSave: (form: ConvenioForm) => Promise<void>
  onClose: () => void
}) {
  const [form, setForm] = useState<ConvenioForm>(convenio ?? empty)
  const [saving, setSaving] = useState(false)
  const { showToast } = useToast()

  const set = (k: Exclude<keyof ConvenioForm, 'activo' | 'id'>) => (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => setForm(f => ({ ...f, [k]: e.target.value }))
...
export default function AdminConvenios() {
  const { showToast } = useToast()
  const [lista, setLista] = useState<ConvenioAdmin[]>([])
  const [loading, setLoading] = useState(true)
  const [modal, setModal] = useState<null | 'new' | ConvenioAdmin>(null) // null | 'new' | convenio obj

  async function fetchLista() {
    try {
      const r = await convenioService.getAll()
      setLista(listaConveniosAdmin(r.data))
  ...
  async function handleSave(form: ConvenioForm) {
    if (form.id) {
      await convenioService.update(form.id, form as JsonBody)
      showToast('Actualizado', 'success')
    } else {
      await convenioService.create(form as JsonBody)
  ...
  async function handleToggle(c: ConvenioAdmin) {
    await convenioService.update(c.id, { ...c, activo: !c.activo } as JsonBody)
