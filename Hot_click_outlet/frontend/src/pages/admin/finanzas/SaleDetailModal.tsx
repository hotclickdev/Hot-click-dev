...
import type { PedidoFinanzas } from './finanzasHelpers'

type DetallePedidoProps = {
  loading: boolean
  error: boolean
  data: PedidoFinanzas | null
}

function DetallePedido({ loading, error, data }: DetallePedidoProps) {
  if (loading) {
    return (
  ...
  const items: ItemPedido[] = data.items ?? []
        ...
        <div>
          ...
          <div className="space-y-2">
            {items.map((item, i) => {
              ...
              const subtotalItem = Number(precio) * Number(item.cantidad ?? 1)
              return (
...
function pedidoDesdeRespuesta(data: unknown): PedidoFinanzas | null {
  if (!data || typeof data !== 'object') return null
  const wrapped = 'data' in data ? (data as { data: unknown }).data : data
  if (!wrapped || typeof wrapped !== 'object') return null
  return wrapped as PedidoFinanzas
}
...
export default function SaleDetailModal({ pedidoId, onClose }: SaleDetailModalProps) {
  const [data, setData] = useState<PedidoFinanzas | null>(null)
  ...
  useEffect(() => {
    ...
    orderService.getById(pedidoId)
      .then((r) => { if (!cancelado) setData(pedidoDesdeRespuesta(r.data)) })
