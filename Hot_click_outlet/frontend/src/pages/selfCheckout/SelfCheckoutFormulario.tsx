...
export default function SelfCheckoutFormulario({
  ...
  return (
    <div className="flex-1 p-4 space-y-4">
      ...
      <div className="rounded-2xl divide-y" style={{ backgroundColor: '#1E242E', divideColor: 'rgba(255,255,255,0.05)' } as CSSProperties}>
        {Object.values(carrito).map(({ producto, cantidad }) => (
          <div key={String(producto.id)} className="flex items-center justify-between px-4 py-3 gap-3">
            ...
            <p className="text-sm font-semibold text-white">₡{fmt((producto.precio as number) * cantidad)}</p>
          </div>
        ))}
