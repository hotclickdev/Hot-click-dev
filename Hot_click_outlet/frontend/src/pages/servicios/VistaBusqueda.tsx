...
import type { TabBusqueda } from './serviciosHelpers'
...
type VistaBusquedaProps = Omit<FormularioBusquedaProps, 't'> & {
    ...
    <motion.div key="busqueda"
      ...
      {token && (
        <div className="flex gap-1 mb-6 p-1 rounded-2xl" style={{ backgroundColor: 'var(--hc-surface)' }}>
          {([{ key: 'solicitar' as const, label: 'Nueva solicitud', mas: true }, { key: 'mis-solicitudes' as const, label: 'Mis solicitudes', mas: false }]).map(tb => (
