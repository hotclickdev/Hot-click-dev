  ...
  return (
    <MainLayout>
      <div className="max-w-xl mx-auto px-4 py-14">
        ...
        >
          <CabeceraPedido esEfectivo={esEfectivo} />
...
function CabeceraPedido({ esEfectivo }: { esEfectivo: boolean }) {
  return (
...
function BloqueEfectivo({
  pagoData, totalFinal, onSinpeWhatsApp,
}: {
  pagoData: PagoDataSinpe | null
  totalFinal: number
  onSinpeWhatsApp: () => void
...
function BloqueSinpe(props: CheckoutSinpePendingProps) {
  const {
    ...
    sinpeInputRef, onSubirComprobante, onSinpeWhatsApp,
  } = props
  ...
  return (
    <>
      ...
      {sinpeUploadEstado !== 'done' ? (
        ...
        <ComprobanteListo onSinpeWhatsApp={onSinpeWhatsApp} />
      )}
...
function SubirComprobante({
  sinpeImagen, setSinpeImagen, sinpeImagenErr, setSinpeImagenErr,
  sinpeUploadEstado, setSinpeUploadEstado, sinpeUploadError, setSinpeUploadError,
  sinpeInputRef, onSubirComprobante,
}: {
  sinpeImagen: File | null
  setSinpeImagen: Dispatch<SetStateAction<File | null>>
  sinpeImagenErr: string
  ...
  return (
    <div className="space-y-3">
      ...
      <button type="button"
        ...
        className="w-full flex items-center justify-center gap-2.5 py-4 rounded-xl text-sm font-medium transition-all border-dashed border-2 min-h-11"
        style={sinpeImagen
...
function ComprobanteListo({ onSinpeWhatsApp }: { onSinpeWhatsApp: () => void }) {
  return (
    <motion.div
...
function LinkMisPedidos({ primario = false }: { primario?: boolean }) {
  if (primario) {
    return (
      <a href="/mis-pedidos" className="hc-btn hc-btn-primary w-full min-h-11 inline-flex items-center justify-center">
        Ver mis pedidos
      </a>
    )
  }
  return (
    <a href="/mis-pedidos" className="block text-xs text-center mt-1 hover:underline" style={{ color: 'var(--hc-muted)' }}>
