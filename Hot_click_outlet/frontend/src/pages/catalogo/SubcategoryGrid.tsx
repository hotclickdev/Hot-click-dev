...
export default function SubcategoryGrid({
  subcats, onSelect, productCountByCat,
}: {
  subcats?: CatalogCategoria[] | null
  onSelect: (id: string) => void
  productCountByCat: CatalogCounts
}) {
  ...
  return (
    <div className="mb-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {subcats.filter(sub => (productCountByCat[String(sub.id)] ?? 0) > 0).map((sub, idx) => {
          const count = productCountByCat[String(sub.id)] ?? 0
