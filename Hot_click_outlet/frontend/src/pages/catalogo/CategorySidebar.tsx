...
function IconoCategoriaNodo({
  nodo, className = 'w-4 h-4 shrink-0',
}: {
  nodo?: CatalogCategoria
  className?: string
}) {
            ...
            <>
              ...
              >
                ...
                <span className="text-[10px] font-bold opacity-50 shrink-0">
                  {categoryTotalCount?.[String(drilledNode.id)] ?? 0}
                </span>
              </button>
              {/* Hijos */}
              <div className="pl-3 mt-0.5 space-y-0.5">
                {drilledNode.children
                  .filter(sub => (categoryTotalCount?.[String(sub.id)] ?? 0) > 0)
                  .map(sub => {
                    const subCount = categoryTotalCount?.[String(sub.id)] ?? 0
                    return (
                    ...
                    .map(cat => (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-bold opacity-50 shrink-0 ml-auto">
                          {categoryTotalCount?.[String(cat.id)] ?? 0}
                        </span>
