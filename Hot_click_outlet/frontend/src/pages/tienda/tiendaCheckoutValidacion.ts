...
export type FormCheckoutTienda = {
  metodoEnvio: string
...
export function faltaDireccionDomicilio(metodoEnvio: string, direccionEntrega: string): boolean {
...
export function faltaTelefono(telefono: string): boolean {
  const digitos = String(telefono ?? '').replace(/\D/g, '')
  return digitos.length < TELEFONO_MIN_DIGITOS
}

export function mensajeErrorCheckout(form: FormCheckoutTienda): string | null {
