...
import type { PedidoCliente, ItemPedidoCliente } from './pedidoHelpers'

type TabPedido = 'detalle' | 'notificaciones'

function TabBar({
  tab, onTab, notifCount,
}: {
  tab: TabPedido
  onTab: (id: TabPedido) => void
  notifCount: number
}) {
  const tabs: TabPedido[] = ['detalle', 'notificaciones']
  return (
  ...
  return (
    <div className="px-5 pb-5 space-y-4 pt-4">
      ...
      <div className="rounded-xl p-3 space-y-1.5" style={{ backgroundColor: 'var(--hc-surface-2)' }}>
        {order.costoEnvio != null && order.costoEnvio > 0 && (
...
export default function OrderCard({ order }: { order: PedidoCliente }) {
  const { t } = useTranslation()
  const [open, setOpen] = useState(false)
  const [tab, setTab] = useState<TabPedido>('detalle')
