...
export default function EmpresaSelectionPage() {
  ...
  const { empresas = [], tempToken = '' } = location.state ?? {}

  const [loading, setLoading] = useState(null) // empresaId being loaded
  const [error, setError]     = useState(null)

  async function seleccionar(empresa) {
    ...
    try {
      ...
      setError(err?.response?.data?.message ?? 'Error al seleccionar el negocio')
      setLoading(null)
    }
  }
