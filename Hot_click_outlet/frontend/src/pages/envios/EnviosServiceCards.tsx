...
import { services, type EnviosCta, type EnviosService } from './enviosData'
...
function claseBadge(tipo: EnviosService['badgeType']) {
  if (tipo === 'official') return 'badge-official'
  if (tipo === 'prior') return 'badge-prior'
  return 'badge-soon'
}

function BadgeServicio({ type, label }: { type: EnviosService['badgeType']; label: string }) {
  return (
...
function CtaServicio({ cta }: { cta?: EnviosCta }) {
  if (!cta) return null
...
function ServiceCard({ service: s }: { service: EnviosService }) {
