...
function Miniatura({ nombre, imagen }: { nombre: string; imagen?: string | null }) {
  if (imagen) {
    return (
      <img
        src={imagen}
        alt=""
        className="size-12 shrink-0 rounded-xl object-cover bg-[var(--hc-surface-2)]"
      />
    )
  }
  ...
  return (
    <span
      className="size-12 shrink-0 rounded-xl grid place-items-center text-xs font-bold tracking-wide"
