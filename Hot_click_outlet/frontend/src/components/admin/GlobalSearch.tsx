...
import {
  ...
  queryParecePedido,
  type ItemBusquedaGlobal,
} from './globalSearchQuery'
import TrustGlyph from '@/components/ui/TrustGlyph'

type ResultadosBusqueda = {
  productos: ItemBusquedaGlobal[]
  pedidos: ItemBusquedaGlobal[]
  clientes: ItemBusquedaGlobal[]
}
...
function ResultGroup({
  ...
  title: string
  items: ItemBusquedaGlobal[]
  onSelect: (item: ItemBusquedaGlobal) => void
}) {
...
export default function GlobalSearch({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [results, setResults] = useState<ResultadosBusqueda>({ productos: [], pedidos: [], clientes: [] })
    ...
    timerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        setResults({
          ...
          clientes: mapearClientesBusqueda(clienList as unknown[]),
        })
      } catch (err: unknown) {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => { setQ(e.target.value); buscar(e.target.value) }

  const handleSelect = (item: ItemBusquedaGlobal) => {
    onClose()
...
async function buscarPedidoSiAplica(query: string): Promise<unknown[]> {
  if (!queryParecePedido(query)) return []
  try {
    const res = await api.get<unknown>(`/pedidos/${query.trim()}`)
    const raw = res.data as { data?: unknown; id?: unknown } | null | undefined
    const p = (raw?.data ?? raw) as { id?: unknown } | undefined
    return p?.id ? [p] : []
  } catch (err: unknown) {
