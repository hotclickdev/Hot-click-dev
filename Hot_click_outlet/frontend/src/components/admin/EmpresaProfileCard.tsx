...
let _cached: PerfilEmpresa | null | undefined = null

function extraerPerfil(data: unknown): PerfilEmpresa | null | undefined {
  const root = data as { id?: unknown; data?: unknown } | null | undefined
  const e = root?.id ? root : (root?.data ?? data)
  return e as PerfilEmpresa | null | undefined
}

function useEmpresaPerfil() {
  const [empresa, setEmpresa] = useState<PerfilEmpresa | null | undefined>(_cached)
  ...
  useEffect(() => {
    ...
    api.get<unknown>('/empresa/perfil')
      .then(r => {
        const e = extraerPerfil(r.data)
        _cached = e
        setEmpresa(e)
      })
      .catch((err: unknown) => { console.error('[EmpresaProfileCard] perfil', err) })
 ...
 */
export default function EmpresaProfileCard({
  totalProductos,
  className = '',
}: {
  totalProductos?: number
