...
type AIChatMessageListProps = {
  ...
  copyMessage: (texto: string, idx: number) => void
...
export function AIChatMessageList({
  ...
  return mensajes.map((m, i) => {
    ...
    const optsVisibles = esUltimo && !m.typing && m.opts && m.opts.length > 0 ? m.opts.slice(0, 2) : []
    return (
    <div
      key={i}
    ...
    >
      ...
      <div className="max-w-[85%] space-y-2">
        ...
        {!m.typing && m.categorias && m.categorias.length > 0 && (
          <div className="flex flex-wrap gap-1.5 pt-0.5">
            {m.categorias.map(cat => (
              <AICategoryChip key={cat} nombre={cat} accentColor={accent} onSelect={undefined} />
            ))}
          </div>
        )}
  ...
  return (
    ...
    >
      <input
        ...
        onFocus={e => { (e.currentTarget.closest('div') as HTMLDivElement).style.borderColor = accent }}
        onBlur={e => { (e.currentTarget.closest('div') as HTMLDivElement).style.borderColor = '#E5E7EB' }}
      />
      {showHumanButton && (
        <a
          ...
          className="w-11 h-11 ml-1 rounded-full shrink-0 flex items-center justify-center transition-opacity hover:opacity-80"
...
type AIChatChipsProps = {
  chips: string[]
  enviar: (chip: string) => void
  accent: string
}

export function AIChatChips({ chips, enviar, accent }: AIChatChipsProps) {
