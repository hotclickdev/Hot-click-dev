...
import { buildInitialMessages, type CheckoutAssistantProps, type MensajeAsistenteCheckout } from './checkoutAssistantHelpers'

type RespuestaChatCheckout = {
  respuesta?: string
...
export function useCheckoutAssistant({ tipo, numeroPedido = '', metodoPago = '', errorCode = '', usuarioDatos = {} }: CheckoutAssistantProps) {
  const [mensajes, setMensajes] = useState<MensajeAsistenteCheckout[]>([])
  ...
  async function enviarDirecto(msg: string) {
    ...
    try {
      const result: RespuestaChatCheckout = await shoppingAssistantService.chat({
