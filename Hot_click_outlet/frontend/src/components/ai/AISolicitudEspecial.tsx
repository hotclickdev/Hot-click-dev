...
export default function AISolicitudEspecial({
  descripcionInicial = '',
  onSuccess,
  onCancel,
}: {
  ...
  const [errors, setErrors] = useState<ErroresSolicitud>({})
  ...
  function setCampo<K extends keyof FormSolicitud>(field: K, value: FormSolicitud[K]) {
    ...
    if (errors[field as keyof ErroresSolicitud]) setErrors(prev => { const next = { ...prev }; delete next[field as keyof ErroresSolicitud]; return next })
  }
