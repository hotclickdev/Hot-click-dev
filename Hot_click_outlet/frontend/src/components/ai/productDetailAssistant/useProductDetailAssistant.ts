...
import type { MensajeAsistenteProducto } from './productDetailAssistantHelpers'

type RespuestaChatProducto = {
  respuesta?: string
...
export function useProductDetailAssistant(product: Producto | null | undefined) {
  const [abierto,  setAbierto]  = useState(false)
  const [mensajes, setMensajes] = useState<MensajeAsistenteProducto[]>([])
  ...
  async function enviar(mensajeDirecto?: string) {
    ...
    try {
      const result: RespuestaChatProducto = await shoppingAssistantService.chat({
        empresaSlug: 'hotclick', mensaje: msg, sesionId, contexto, visitorId,
        productoId: (product?.id ?? null) as Id | undefined,
      })
