...
export default function CheckoutStepper({ activeStep = 'checkout' }: { activeStep?: StepId }) {
            ...
            <div className="flex flex-col items-center gap-1.5">
              {puedeVolver ? (
                <Link to={step.href} aria-label={t('checkoutStepper.backTo', { step: label })}>
                  <StepCircle done active={false} index={i + 1} />
                </Link>
              ) : (
...
function StepCircle({ done, active, index }: { done: boolean; active: boolean; index: number }) {
  let circleStyle: CSSProperties = { background: 'color-mix(in srgb, var(--hc-surface) 60%, transparent)', color: 'color-mix(in srgb, var(--hc-muted) 40%, transparent)', border: '1px solid var(--hc-border)' }
