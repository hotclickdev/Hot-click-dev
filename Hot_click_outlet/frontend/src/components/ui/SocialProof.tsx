...
export default function SocialProof({ productId }: { productId?: Id }) {
  ...
  useEffect(() => {
    ...
    testimonioService.getRating(productId)
      .then(({ data }) => {
        const envelope = data as { data?: RatingInfo } | RatingInfo | undefined
        const d = envelope && typeof envelope === 'object' && 'reviewCount' in envelope
          ? envelope
      ...
      .catch((err: unknown) => { console.error('[SocialProof] rating', err) })
  }, [productId])
