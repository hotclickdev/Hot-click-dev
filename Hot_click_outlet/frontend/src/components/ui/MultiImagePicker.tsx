...
function extraerUrlSubida(payload: unknown): unknown {
  ...
  const nestedUrl = nested && typeof nested === 'object'
    ...
    : undefined
  return nestedUrl ?? obj.url ?? payload
}

function mensajeFallo(reason: unknown): string | undefined {
...
async function uploadImagen(file: File): Promise<string> {
  ...
  const url = extraerUrlSubida(r.data)
  if (!url || typeof url !== 'string') throw new Error('No se obtuvo URL de la imagen')
  return url
}

export type MultiImagePickerProps = {
  imagenes?: string[]
  onChange: (urls: string[]) => void
}

export default function MultiImagePicker({ imagenes = [], onChange }: MultiImagePickerProps) {
  ...
  const processFiles = async (fileList: FileList | null) => {
    ...
    const failed  = results.filter((r) => r.status === 'rejected').map((r) => mensajeFallo(r.reason))
    if (failed.length > 0) setError(failed[0] ?? '')
    if (newUrls.length > 0) onChange([...imagenes, ...newUrls])
    setUploadingCount(0)
  }

  const remove = (idx: number) => onChange(imagenes.filter((_, i) => i !== idx))
