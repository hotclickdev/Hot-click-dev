...
import type { CartaProductoDescubri, DirSwipeDescubri } from '@/pages/descubri/destinoDetalleDescubri'

type SwipeCardProps = {
  product: CartaProductoDescubri
  isTop: boolean
  stackIndex: number
  onSwipe: (dir: DirSwipeDescubri) => void
}

export default function SwipeCard({ product, isTop, stackIndex, onSwipe }: SwipeCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex-1 min-h-0 bg-white flex items-center justify-center p-6">
        ...
        {product.enOferta && (product.porcentajeDescuento ?? 0) > 0 && (
          <span
