...
type MovimientoKardex = {
  ...
  stockActualAntes?: number
  stockActualDespues?: number
  referencia?: string
  operadorCorreo?: string
...
export default function KardexDrawer({ producto, onClose }: {
  ...
  useEffect(() => {
    ...
    productService.kardex(producto.id)
      .then((lista) => setMovimientos(lista as MovimientoKardex[]))
                ...
                <div key={m.id ?? i} className="rounded-xl p-3 flex items-center gap-3"
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <span className="text-xs ml-1"
                      style={{ color: Number(m.stockActualDespues) >= Number(m.stockActualAntes) ? '#34d399' : '#f87171' }}>
                      ({Number(m.stockActualDespues) >= Number(m.stockActualAntes) ? '+' : ''}{Number(m.stockActualDespues) - Number(m.stockActualAntes)})
                    </span>
                  </div>
