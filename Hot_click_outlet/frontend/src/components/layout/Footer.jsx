import { Link } from 'react-router-dom'
import { motion } from 'framer-motion'
import { useTranslation } from 'react-i18next'

const footerLinks = (t) => [
  {
    title: t('footer.tienda'),
    links: [[t('footer.productos'), '/productos'], [t('footer.carrito'), '/carrito']],
  },
  {
    title: t('footer.empresa'),
    links: [[t('footer.nosotros'), '/nosotros'], [t('footer.contacto'), '/contacto']],
  },
  {
    title: t('footer.cuenta'),
    links: [
      [t('footer.iniciarSesion'), '/login'],
      [t('footer.registrarse'), '/registro'],
      [t('footer.miPerfil'), '/perfil'],
    ],
  },
]

export default function Footer() {
  const { t } = useTranslation()

  return (
    <footer className="mt-auto pb-16 md:pb-0" style={{ borderTop: '1px solid var(--hc-border)', backgroundColor: 'var(--hc-surface)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.55, ease: [0.16, 1, 0.3, 1] }}
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12"
        >
          {/* Brand */}
          <div className="col-span-1 sm:col-span-2 lg:col-span-1">
            <Link to="/" className="flex items-center gap-2 mb-4 group w-fit">
              <div className="flex items-center justify-center w-8 h-8 rounded-xl shrink-0 hc-logo-badge transition-transform duration-200 group-hover:scale-105">
                <span className="text-white text-[12px] leading-none" style={{ fontFamily: "'Barlow', sans-serif", fontWeight: 900 }}>HC</span>
              </div>
              <span className="text-[18px] leading-none uppercase hc-logo-text"
                style={{ fontFamily: "'Barlow', sans-serif", fontWeight: 900 }}>
                HOTCLICK
              </span>
            </Link>
            <p className="text-sm leading-relaxed max-w-xs" style={{ color: 'var(--hc-muted)', lineHeight: '1.7' }}>
              {t('footer.tagline')}
            </p>
            <div className="flex items-center gap-3 mt-6">
              <a
                href="https://wa.me/50689745370"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="WhatsApp"
                className="p-2.5 rounded-xl transition-all duration-200 hover:scale-110 hover:-translate-y-0.5"
                style={{ color: 'var(--hc-muted)', background: 'var(--hc-surface-2)', border: '1px solid var(--hc-border)' }}
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Link columns */}
          {footerLinks(t).map(({ title, links }) => (
            <div key={title}>
              <h3
                className="text-[10px] font-bold uppercase tracking-widest mb-5"
                style={{ color: 'var(--hc-accent)' }}
              >
                {title}
              </h3>
              <ul className="space-y-3">
                {links.map(([label, href]) => (
                  <li key={href}>
                    <Link
                      to={href}
                      className="hc-underline-hover text-sm transition-colors duration-200"
                      style={{ color: 'var(--hc-muted)' }}
                      onMouseEnter={e => e.currentTarget.style.color = 'var(--hc-text)'}
                      onMouseLeave={e => e.currentTarget.style.color = 'var(--hc-muted)'}
                    >
                      {label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </motion.div>

        <div
          className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-3"
          style={{ borderTop: '1px solid var(--hc-border)' }}
        >
          <p className="text-xs" style={{ color: 'var(--hc-muted)', opacity: 0.75 }}>
            © {new Date().getFullYear()} HOTCLICK · {t('footer.derechos')}
          </p>
          <p className="text-xs flex items-center gap-1.5" style={{ color: 'var(--hc-muted)', opacity: 0.75 }}>
            <span>Hecho con amor en</span>
            <span>Costa Rica 🇨🇷</span>
          </p>
        </div>
      </div>
    </footer>
  )
}
