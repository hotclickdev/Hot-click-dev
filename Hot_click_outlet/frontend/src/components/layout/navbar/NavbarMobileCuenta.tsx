...
/** Sección Mi cuenta / auth del menú móvil. */
export default function NavbarMobileCuenta({
  token,
  userName,
  isAdmin,
  setMenuOpen,
  onLogout,
}: Pick<NavbarMobileMenuProps, 'token' | 'userName' | 'isAdmin' | 'setMenuOpen' | 'onLogout'>) {
      ...
      {token ? (
        <>
          <p className="text-[10px] font-bold tracking-[0.18em] uppercase px-3 mb-2"
            style={{ color: 'var(--hc-muted)' }}>{t('nav.miCuenta')}</p>
          {/* Avatar */}
          <div className="flex items-center gap-3 px-3 py-3 mb-2 rounded-xl"
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold truncate" style={{ color: 'var(--hc-text)' }}>{userName || t('nav.usuario')}</p>
              <p className="text-xs" style={{ color: 'var(--hc-muted)' }}>{t('nav.cuentaActiva')}</p>
            </div>
          </div>
          <div className="flex flex-col gap-0.5">
            {[
              { to: '/perfil', label: t('footer.miPerfil'), icon: <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /> },
              { to: '/mis-pedidos', label: t('nav.misPedidos'), icon: <path strokeLinecap="round" strokeLinejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /> },
            ].map(({ to, label, icon }) => (
            ...
            {isAdmin() && (
              <Link to={rutaPanel} onClick={() => setMenuOpen(false)}
                className="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-150"
                style={{ color: 'var(--hc-muted)' }}
                onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'var(--hc-surface-2)'; e.currentTarget.style.color = 'var(--hc-text)' }}
                onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = 'var(--hc-muted)' }}
            ...
            >
              ...
              <span className="text-sm font-medium">{t('nav.cerrarSesion')}</span>
            </button>
          </div>
        </>
      ) : (
