...
export function useNavbar() {
  ...
  const [categoriasPadre, setCategoriasPadre] = useState<CategoriaNavbar[]>([])

  const loadCategorias = useCallback(() => {
    if (categoriasPadre.length > 0) return
    api.get('/categorias/publicas')
      .then(({ data }) => {
        const all = Array.isArray(data) ? data as CategoriaNavbar[] : []
        setCategoriasPadre(all.filter((c) => !c.padreId))
      })
      .catch((err: unknown) => { console.error('[useNavbar] categorias', err) })
