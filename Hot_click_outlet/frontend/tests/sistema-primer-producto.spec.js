...
function perfil(estadoEmpresa, visibilidadPublica) {
  return {
    id: 1,
    estadoEmpresa,
    visibilidadPublica,
...
async function mockApis(page, { estadoEmpresa = 'ACTIVO', visibilidadPublica = true } = {}) {
...
async function sesion(page, opts) {
  ...
  await page.addInitScript((auth) => {
    ...
    globalThis.__hcClipboard = ''
    const escribir = async (texto) => { globalThis.__hcClipboard = String(texto) }
    if (navigator.clipboard) {
      navigator.clipboard.writeText = escribir
    } else {
      Object.defineProperty(navigator, 'clipboard', {
        configurable: true,
        value: { writeText: escribir, readText: async () => globalThis.__hcClipboard },
      })
    }
  }, payloadAuth())
}

async function esperarLinkCopiado(page) {
  ...
  const clip = await page.evaluate(() => globalThis.__hcClipboard)
  expect(clip).toMatch(/\/tienda\/demo$/)
}

async function publicarProducto(page) {
